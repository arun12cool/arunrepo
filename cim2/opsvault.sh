...
vault write $acc/sign/$acc-$user   public_key=@$HOME/.ssh/authorized_keys >/dev/null

vault write -field=signed_key $acc/sign/$acc-$user   public_key=@$HOME/.ssh/authorized_keys > .ssh/id_rsa-cert.pub
...
echo " Command : ssh -i .ssh/id_rsa-cert.pub  username@IP"
